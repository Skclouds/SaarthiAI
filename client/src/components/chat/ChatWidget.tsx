...
export default function ChatWidget({
  ...
  const loadConfig = useCallback(async () => {
    try {
      ...
      setSuggestedQuestions(['Track my order', 'Pricing', 'Refund policy', 'Contact support']);
      setInitialized(true);
    }
  }, [businessId]);
      ...
      <AnimatePresence>
        {open && (
          ...
          >
              ...
              <div className="relative flex items-center justify-between px-5 py-4 text-white">
                <div className="flex items-center gap-3">
                  <SaarthiLogo
                    ...
                    size={40}
                    rounded="full"
                    className="border border-white/20 shadow-soft"
                  />
                  <div>
                    <p className="font-semibold text-body">{botName}</p>
                ...
                <button
                  ...
                  className="p-2 rounded-xl hover:bg-white/10 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40"
            ...
            {/* Messages */}
            <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5 bg-gradient-to-b from-background to-surface-muted/30">
                ...
                >
                  {msg.role === 'ASSISTANT' && (
                    <SaarthiLogo
                      ...
                      size={32}
                      rounded="full"
                      className="mt-0.5 border border-border/60"
                    />
                  )}
                  <div
                    className={cn(
              ...
              {sources.length > 0 && messages.length > 0 && !loading && (
                <div className="chat-source-card mx-1">
            ...
            <form onSubmit={handleSubmit} className="px-5 py-4 border-t border-border/60 bg-surface shrink-0">
              <div className="flex items-center gap-2.5">
                <input
                  ...
                  disabled={loading}
                  className="flex-1 px-4 py-3 rounded-xl border border-border bg-background text-body text-navy-900 placeholder:text-navy-400 focus:outline-none focus:ring-2 focus:ring-brand-accent/30 focus:border-brand-accent disabled:opacity-50 transition-all"
                />
                <motion.button
                  type="submit"
                  ...
                  whileTap={reduced ? undefined : { scale: 0.95 }}
                  className="p-3 rounded-xl bg-gradient-to-br from-brand-deep to-brand-accent text-white disabled:opacity-40 shadow-soft transition-opacity"
                >
